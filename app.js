...
function productCategory(data){
  ...
  thumbnail.alt = data.brand;
  //Appending product image into card
  card.appendChild(thumbnail);

  //Creating Product details section
  let details = document.createElement("div");
  details.className = "details";
  //Appending details section to card
  card.appendChild(details);

  //Creating Product Title
  let prodTitle = document.createElement("h3");
  prodTitle.className = "product-title";
  prodTitle.innerHTML = data.name;
  //appending to details section
  details.appendChild(prodTitle);

  //product brand
  let prodBrand = document.createElement("h4");
  prodBrand.className = "product-brand";
  prodBrand.innerHTML = data.brand;
  details.appendChild(prodBrand);

  //product price
  let prodPrice = document.createElement("p");
  prodPrice.className = "product-price";
  prodPrice.innerHTML = "Rs "+data.price;
  details.appendChild(prodPrice);


  if(data.isAccessory === false){
    let category = document.getElementById("clothing-wrapper");
    category.appendChild(card);
    return card;
  }else{
    let category = document.getElementById("accessories-wrapper");
    category.appendChild(card);
  }
